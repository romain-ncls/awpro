...
pub fn open(api: &HidApi) -> Result<HidDevice, AppError> {
    api.open(VENDOR_ID, PRODUCT_ID).map_err(|_| AppError::DeviceNotFound)
}

/// Send a 62-byte feature report (Report ID 6).
/// `payload` holds only the meaningful bytes; the rest are zeroed.
/// After sending, drains any pending device response so it doesn't
/// pollute a subsequent query.
pub fn write(device: &HidDevice, payload: &[u8]) -> Result<(), AppError> {
    ...
    device.send_feature_report(&buf).map_err(|e| AppError::HidWrite(e.to_string()))?;
    drain(device);
    Ok(())
}
...
pub fn query(device: &HidDevice, cmd: u8) -> Result<[u8; REPORT_LEN + 1], AppError> {
    ...
    req[1] = 0xC0;
    req[2] = cmd;
    device.send_feature_report(&req).map_err(|e| AppError::HidWrite(e.to_string()))?;
